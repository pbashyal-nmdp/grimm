...
class CypherQuery(object):
    ...
    def getMatch(self, typing):
        ...
        match = "MATCH ("+loci_lc+":Haplotype)"
        match_clause = f"{loci_lc}.loci=\"{loci_lc.upper()}\""
        return match, match_clause
    ...
    def haplosIn(self, haplos):
        ...
        loci_lc = "".join([allele.lower() for allele in loci])
        where_clause = " AND " + loci_lc + ".name IN [ " + haplo_quoted + " ]"
        return where_clause
    ...
    def getPath(self, typing):
        ...
        for missing_locus in missing:
            ...
            nodeName = "(" + loci_lc + ":Haplotype)"
            path_clause = f'{loci_lc}.loci = \"{loci_up}\"'
            nodes.append((nodeName, path_clause))
        ...
        path_clause = " AND ".join([node[1] for node in nodes])
        return(query_path, path_clause)

    def buildQuery(self, typing):
        alleles = typing[0].split("~")
        # If there are missing loci...
        if len(alleles) < len(self.loci_a):
            match, match_clause = self.getMatch(typing)
            path, path_clause = self.getPath(typing)
            hapsin = self.haplosIn(typing)
            query = match + '-' + path + ' WHERE ' + match_clause + ' AND ' + path_clause + hapsin + " " +  self.return_freq
            return query
        else:
            match, match_clause = self.getMatch(typing)
            hapsin = self.haplosIn(typing)
            query = match + ' WHERE ' + match_clause + hapsin + " " +  self.return_freq
            return query
