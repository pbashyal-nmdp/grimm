...
header = ['haplotype_id', 'name', 'loci', 'frequency']
node_file = 'output/csv/nodes.csv'
with open(node_file, mode='wt') as csvfile:
    ...
    if loci != FULL_LOCI:
        ...
        for haplotype, haplotype_node in list(loci_map.items()):
            for parent in haplotype_node.parents:
                ...
                l = list([haplotype_node.node_id, parent_id, prob_array])
                fq_l.extend([l])

freq_df = pd.DataFrame(fq_l, columns=["HapNode", "ParentID", "P"])
ndf = freq_df.join(freq_df.groupby(['HapNode', 'ParentID'])['P'].sum(), on=['HapNode', 'ParentID'], rsuffix='_r')
df2 = ndf[['HapNode', 'ParentID', 'P_r']]
df2 = df2.drop_duplicates()
df2.columns = ['start_haplotype_id', 'end_haplotype_id', 'cp']
df2.to_csv(edge_file, index=False, header=True, lineterminator='\n')

# # #### Generate Top Links file
header = ['start_haplotype_id', 'end_haplotype_id']
        ...
        if loci != FULL_LOCI: # FULL_LOCI is already in the dictionary
            ...
            for haplotype, haplotype_node in list(loci_map.items()):
                ...
                for top_link in top_links:
                    csv_writer.writerow([haplotype_node.node_id, top_link.node_id])
